...
| 근거 문서 | `project_purpose.md`, `docs/analysis.md`, `README.md`, `.cursorrules` |
...
**현재 상태** (`docs/analysis.md` §1, §3, §6):

| 문제 영역 | 구체적 내용 | 근거 |
|-----------|-------------|------|
| God Function | `main.cpp`에 HTTP 라우팅·HTML 렌더링·CSV 파싱·오케스트레이션이 혼재 (372줄) | `docs/analysis.md` §6 M-1, `project_purpose.md` §4.2 |
| 전역 상태 | `Session::currentFeedbacks`, `fil_data`, `TextAnalyzer::globalSent/globalKw` | `docs/analysis.md` §4.3, §6 M-2 |
| 중복 로직 | `containsAny`가 TextAnalyzer·Filters에 각각 존재 | `docs/analysis.md` §6 M-3, `project_purpose.md` §4.1 |
| 감정 키워드 이중 관리 | `Constants::SENTIMENT_KEYWORDS` vs `Filters::S_KEYWORDS` | `docs/analysis.md` §6 H-1, M-7 |
| P0 기능 버그 | H-1 중립 필터 불일치, H-2 CSV `text` 컬럼 미사용, H-3 키워드 필터 `main` 누락 | `docs/analysis.md` §6.1, §7 |
| 테스트 부재 | Google Test·CMake 테스트 타깃 0건 | `docs/analysis.md` §5.3, §6 H-4 |
...
| ID | 목표 | 측정 기준 | 근거 |
|----|------|-----------|------|
| G-1 | HTTP API 계약 유지 | 5개 엔드포인트(path·method) 변경 0건; 부록 체크리스트 5항목 통과 | `docs/analysis.md` 부록, `.cursorrules` §3 |
| G-2 | P0 버그 수정 및 회귀 방지 | H-1/H-2/H-3 각각 재현 단위 테스트 1건 이상 + Green | `docs/analysis.md` §6.1, §8 Phase 1 |
| G-3 | Service 계층 테스트 커버리지 | TextAnalyzer·Filters·CsvParser **≥ 90%** (gcov/lcov 또는 동등) | `project_purpose.md` §6.1-2, `.cursorrules` §5 |
| G-4 | main.cpp 책임 축소 | HTML·핸들러 분리 후 `main.cpp` **≤ 200줄** | `docs/analysis.md` §8 Phase 2 |
| G-5 | Logger level별 UI 표시 | warning·error가 HTML alert에 반영; success는 타임스탬프 포함 | `project_purpose.md` §6.1-3, `docs/analysis.md` §6 M-4 |
...
| NG-2 | 다중 사용자 인증·권한 시스템 | 로컬 실습·리팩토링 챌린지 범위 (`docs/analysis.md` §6 L-5) |
...
1. Google Test 타깃 추가 후 TextAnalyzer·Filters·CsvParser 단위 테스트 작성 (`docs/analysis.md` §8 Phase 1)
2. H-1/H-2/H-3 failing test → 수정 → Green (`docs/analysis.md` §7 P0)
3. Green 상태 유지하며 HtmlRenderer·RouteHandlers 분리 (`docs/analysis.md` §8 Phase 2)
4. 전역 상태 캡슐화·네이밍 개선 (`docs/analysis.md` §8 Phase 3)
...
| 우선순위 | 기능 ID | 기능 | 설명 | 근거 |
|----------|---------|------|------|------|
| **필수** | F-01 | 대시보드 | GET `/` — UTF-8 한국어 HTML 대시보드 | `docs/analysis.md` §4.1 |
| **필수** | F-02 | 텍스트 분석 | POST `/analyze` — 피드백 추가 + 감정·키워드 집계 | `project_purpose.md` §2.1 |
| **필수** | F-03 | CSV 업로드 | POST `/upload` — 헤더 `text` 컬럼 파싱 | `README.md` CSV 형식, `docs/analysis.md` §6 H-2 |
| **필수** | F-04 | 필터 | POST `/filter` — 감정·키워드 필터 + 집계 | `project_purpose.md` §2.1 |
| **필수** | F-05 | CSV 다운로드 | GET `/download` — UTF-8 BOM CSV | `.cursorrules` §3 |
| **필수** | F-06 | 분류 규칙 단일화 | TextAnalyzer·Filters가 동일 감정·키워드 매칭 규칙 사용 | `docs/analysis.md` §6 H-1, H-3, M-7 |
| **권장** | F-07 | 단위 테스트 | Google Test + CMake `ctest` 타깃 | `docs/analysis.md` §6 H-4 |
| **권장** | F-08 | 관심사 분리 | HtmlRenderer·RouteHandlers·CsvParser 분리 | `docs/analysis.md` §8 Phase 2 |
| **권장** | F-09 | 상태 캡슐화 | 전역 상태 → AppState/Repository | `docs/analysis.md` §8 Phase 3 |
| **권장** | F-10 | Logger UI 연동 | level별(warning/error/success) alert 표시 | `project_purpose.md` §6.1-3 |
| **권장** | F-11 | 멀티라인 입력 검증 | textarea 개행 포함 입력·저장·분석 E2E 검증 | `project_purpose.md` §6.1-3, `docs/analysis.md` §6 L-6 |
...
| **선택** | F-14 | 업로드 검증 | localhost 바인딩 옵션, CSV MIME·크기 제한 | `docs/analysis.md` §6 L-5 |
...
| Response Body | HTML 대시보드: 제목 "Feedback Analyzer", 섹션(피드백 입력·CSV 업로드·필터·분석 결과), 한국어 UI (`docs/analysis.md` 부록) |
| Side Effect | 세션 상태 초기화 호출 (`docs/analysis.md` §4.1) |
...
| 분류 규칙 | F-06과 동일 (`docs/analysis.md` §6 H-1) |
...
| **다운로드 대상 (PRD 확정)** | **마지막 POST `/filter` 성공(≥1건 반환) 직후의 필터 결과 집합** (`docs/analysis.md` §4.3 `fil_data`, §6 M-6 개선 방향 반영) |
...
| 규칙 | 내용 |
|------|------|
| 감정 단일 소스 | `Constants::SENTIMENT_KEYWORDS`만 사용; `Filters::S_KEYWORDS`·`Filters::initFilterKeywords()` 제거 | `docs/analysis.md` §6 M-7 |
| 감정 판정 순서 | 긍정 키워드 매칭 → 부정 키워드 매칭 → 그 외 **중립** (TextAnalyzer·Filters 동일) | `docs/analysis.md` §6 H-1 |
| 키워드 카테고리 매칭 | `Constants::CATEGORY_KEYWORDS[카테고리]["main"]` 키워드 포함 시 해당 카테고리 매칭 (집계·필터 동일 규칙) | `docs/analysis.md` §6 H-3 |
...
| C-04 | CSV 출력: BOM + `text\n` + 본문 | `.cursorrules` §3, `docs/analysis.md` 부록 |
| C-05 | 서버 URL: `http://localhost:8080`; 바인딩: `0.0.0.0:8080` | `README.md`, `docs/analysis.md` §5.2 |
...
| 항목 | 요구 | 근거 |
|------|------|------|
| 언어 | C++17 | `README.md` 요구사항, `docs/analysis.md` §2 |
| 빌드 | CMake 3.14+ | `README.md`, `docs/analysis.md` §2 |
| HTTP | cpp-httplib (헤더 전용) | `docs/analysis.md` §2 |
...
- **리팩토링 원칙:** 테스트 Green → 최소 diff → Extract Function/Class (`docs/analysis.md` §8, `.cursorrules` §6)
...
| HTTP 핸들러 | 통합/스모크 또는 부록 체크리스트 수동 검증 | 5항목 | `docs/analysis.md` 부록 |
...
| **Constants::SENTIMENT_KEYWORDS** | map: `긍정`/`중립`/`부정` → vector\<string\> | Y | 감정 키워드 **단일 소스** | `.cursorrules` §3, `docs/analysis.md` §6 M-7 |
| **Constants::CATEGORY_KEYWORDS** | map: 카테고리 → map: `main`/`sub` → vector\<string\> | Y | 키워드 카테고리 | `Constants.cpp`, `docs/analysis.md` §4.2 |
...
| 상태 | 저장소 | 수명 | 용도 |
|------|--------|------|------|
| 전체 피드백 | `Session::currentFeedbacks` | 프로세스 수명, 인메모리 | `/analyze`, `/upload` 누적 (`docs/analysis.md` §4.3) |
| 마지막 필터 결과 | `fil_data` → Phase 3 `AppState::lastFilteredFeedbacks` | `/filter` 성공 시 갱신 | GET `/download` 소스 (§3.2 F-05) |
| 분석 캐시 (제거 대상) | `TextAnalyzer::globalSent/globalKw` | — | Phase 3에서 제거 (`docs/analysis.md` §8 Phase 3) |
| 미사용 (제거 대상) | `Session::internalData`, `filterOptions` | — | Phase 3 정리 (`docs/analysis.md` §6 L-2) |

### 5.3 CSV 계약

| 방향 | 스키마 | 규칙 |
|------|--------|------|
| **입력** | 헤더: `text` (필수); 추가 컬럼 허용·무시 | `README.md` CSV 형식, `docs/analysis.md` §6 H-2 |
...
| Content-Type | `text/html; charset=UTF-8` | `docs/analysis.md` 부록 |
...
- [ ] **AC-1:** POST `/filter` sentiment=`중립` 결과 집합 = TextAnalyzer 감정 분류 `중립` 집합과 **100% 일치** (H-1, `docs/analysis.md` §6 H-1)
- [ ] **AC-2:** POST `/upload` 시 CSV 헤더 `text` 컬럼 값만 Feedback으로 적재; `text` 없으면 0건 + error/warning (`docs/analysis.md` §6 H-2)
- [ ] **AC-3:** POST `/filter` keyword=`배송` 시 `main` 키워드("배송","택배" 등)만 포함한 피드백 **포함** (H-3, `docs/analysis.md` §6 H-3)
- [ ] **AC-4:** `ctest --test-dir build --output-on-failure` 전체 Green; TextAnalyzer·Filters·CsvParser 커버리지 **≥ 90%** (`project_purpose.md` §6.1-2)
- [ ] **AC-5:** `docs/analysis.md` 부록 엔드포인트 체크리스트 5항목 통과
- [ ] **AC-6 (권장):** warning·error Logger 호출 시 HTML alert에 동일 메시지 표시 (`project_purpose.md` §6.1-3)
- [ ] **AC-7 (권장):** textarea 개행 포함 입력 → Session 저장 → CSV 다운로드 시 개행 유지 (`docs/analysis.md` §6 L-6)
...
| **Session** | `Session::currentFeedbacks` 등 인메모리 전역 상태를 보관하는 클래스 (`docs/analysis.md` §4.3) |
...
`project_purpose.md` §6.1 8단계와 `docs/analysis.md` §8 Phase 1~5 대응표.
